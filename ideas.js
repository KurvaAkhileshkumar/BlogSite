...
function loadData(jsonData){    
    ...
    {        
        if(left==true && jsonData[i].filter=="Ideas")
        {
            ...
            ideasIds.push(jsonData[i].id) 
            left=false
            right=true
        }
        else if(right==true && jsonData[i].filter=="Ideas")
        {
...
function foo(){
    ...
    {
        let chhh=document.querySelector('#C'+ideasIds[i]) 
        console.log(chhh)       
        chhh.addEventListener('click',()=>{
            location.href=`blogPostPage.html?id=${ideasIds[i]}`            
        })
