...
function loadData(jsonData){    
    ...
    {        
        if(left==true && jsonData[i].filter=="Technology")
        {
            ...
            technologyIds.push(jsonData[i].id) 
            left=false
            right=true
        }
        else if(right==true && jsonData[i].filter=="Technology")
        {
...
function foo(){    
    ...
    {
        let chhh=document.querySelector('#C'+technologyIds[i]) 
        console.log(chhh)       
        chhh.addEventListener('click',()=>{
            location.href=`blogPostPage.html?id=${technologyIds[i]}`            
        })
